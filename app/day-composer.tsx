      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 36 }}>
        ...
        <View className="mt-3 flex-row flex-wrap gap-2">
          {snapshot.members.filter((member) => member.active !== false).map((member) => {
            ...
            const webDragProps = Platform.OS === "web" ? ({ draggable: true, onDragStart: () => setSelectedMemberId(member.id) } as any) : {};
            return <Pressable key={member.id} {...webDragProps} onPress={() => setSelectedMemberId(active ? null : member.id)} accessibilityRole="button" accessibilityLabel={`Sélectionner ou faire glisser ${member.name}`} style={({ pressed }) => ({ opacity: pressed ? 0.72 : 1 })}><View style={{ borderColor: active ? member.color : "#D8E0E5", backgroundColor: active ? `${member.color}18` : "#FFFFFF" }} className="rounded-full border px-3 py-2 flex-row items-center gap-2"><View style={{ backgroundColor: member.color }} className="h-7 w-7 rounded-full items-center justify-center"><Text className="text-[10px] font-bold text-white">{member.name.split(" ").map((part) => part[0]).join("").slice(0, 2)}</Text></View><Text className="text-sm font-bold text-foreground">{member.name.split(" ")[0]}</Text>{unavailableAllDay && <Text className="text-[10px] font-bold text-warning">Indispo.</Text>}</View></Pressable>;
          })}
        </View>
