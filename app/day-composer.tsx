      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 36 }}>
        ...
        <View className="mt-3 flex-row flex-wrap gap-2">
          {snapshot.members.filter((member) => member.active !== false).map((member) => {
            ...
            return <Pressable key={member.id} onPress={() => setSelectedMemberId(active ? null : member.id)} accessibilityRole="button" accessibilityLabel={`Sélectionner ${member.name}`} style={({ pressed }) => ({ opacity: pressed ? 0.72 : 1 })}><View style={{ borderColor: active ? member.color : "#D8E0E5", backgroundColor: active ? `${member.color}18` : "#FFFFFF" }} className="rounded-full border px-3 py-2 flex-row items-center gap-2"><View style={{ backgroundColor: member.color }} className="h-7 w-7 rounded-full items-center justify-center"><Text className="text-[10px] font-bold text-white">{member.name.split(" ").map((part) => part[0]).join("").slice(0, 2)}</Text></View><Text className="text-sm font-bold text-foreground">{member.name.split(" ")[0]}</Text>{unavailableAllDay && <Text className="text-[10px] font-bold text-warning">Indispo.</Text>}</View></Pressable>;
          })}
        </View>
...
function ServiceSlot({ index, staffMemberId, shift, selectedMember, saving, onAssign, onRemove, onSaveTime }: { index: number; staffMemberId: number | null; shift?: PlanningShift; selectedMember: Staff | null; saving: boolean; onAssign: () => void; onRemove: () => void; onSaveTime: (staffMemberId: number, startsAt: string, endsAt: string) => Promise<void> }) {
  ...
  if (!member || !staffMemberId) {
    return <Pressable disabled={saving} onPress={onAssign} accessibilityRole="button" accessibilityLabel={`Case ${index + 1} vide${selectedMember ? `, affecter ${selectedMember.name}` : ""}`} style={({ pressed }) => ({ opacity: pressed ? 0.72 : saving ? 0.55 : 1 })}><View className="rounded-2xl border-2 border-dashed border-[#B9D9ED] bg-white px-4 py-4 flex-row items-center gap-3"><View className="h-9 w-9 rounded-xl bg-[#E4F1FB] items-center justify-center"><IconSymbol name="plus" size={19} color="#006491" /></View><View className="flex-1"><Text className="font-bold text-foreground">Case {index + 1} · vide</Text><Text className="mt-1 text-xs text-muted">{selectedMember ? `Placer ${selectedMember.name}` : "Touchez après avoir sélectionné un salarié."}</Text></View></View></Pressable>;
  }
