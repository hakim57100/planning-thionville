...
type DuplicateWeekUndoInfo = { sourceWeekStart: string; targetWeekStart: string; copiedShiftCount: number } | null;
type CancelDuplicatedWeekResult = { sourceWeekStart: string; weekStart: string; removedShiftCount: number };
type ClearDraftWeekResult = { weekStart: string; removedShiftCount: number };
...
type PlanningContextValue = {
  ...
  createShift: (input: ShiftInput) => Promise<void>; importPlanningExcel: (input: ExcelImportInput) => Promise<ExcelImportResult>; updateShift: (id: number, input: Partial<ShiftInput>) => Promise<void>; deleteShift: (id: number) => Promise<void>; clearDraftWeek: () => Promise<ClearDraftWeekResult>; createStaffMember: (input: StaffInput) => Promise<string | null>; updateStaffMember: (id: number, input: StaffInput) => Promise<void>; regenerateStaffCode: (id: number) => Promise<string | null>; setStaffActive: (id: number, active: boolean) => Promise<void>; duplicateWeekToNext: () => Promise<DuplicateWeekResult>; duplicateWeekUndoInfo: DuplicateWeekUndoInfo; cancelDuplicatedWeek: () => Promise<CancelDuplicatedWeekResult>; weekTemplates: WeekTemplate[]; saveWeekAsTemplate: (name: string) => Promise<SaveWeekTemplateResult>; renameWeekTemplate: (id: number, name: string) => Promise<void>; deleteWeekTemplate: (id: number) => Promise<void>; applyWeekTemplate: (id: number) => Promise<ApplyWeekTemplateResult>; publishWeek: () => Promise<PlanningPublicationResult>;
...
export function PlanningProvider({ children }: { children: ReactNode }) {
  ...
  const duplicateWeekUndoInfo = useMemo<DuplicateWeekUndoInfo>(() => isDemo || role !== "admin" ? null : duplicateWeekUndoInfoQuery.data ?? null, [duplicateWeekUndoInfoQuery.data, isDemo, role]);
  ...
  const refetch = useCallback(async () => { if (isAuthenticated) await Promise.all([remoteQuery.refetch(), notificationsQuery.refetch(), authQuery.data?.role === "admin" ? prePublishCheckQuery.refetch() : Promise.resolve(undefined), authQuery.data?.role === "admin" ? duplicateWeekUndoInfoQuery.refetch() : Promise.resolve(undefined)]); }, [authQuery.data?.role, duplicateWeekUndoInfoQuery, isAuthenticated, notificationsQuery, prePublishCheckQuery, remoteQuery]);
  ...
  const clearDraftWeek = useCallback(async (): Promise<ClearDraftWeekResult> => {
    if (isDemo) throw new Error("Connectez-vous avec un compte administrateur pour effacer un brouillon.");
    const result = await clearDraftWeekMutation.mutateAsync({ weekStart });
    await refetch();
    return result;
  }, [clearDraftWeekMutation, isDemo, refetch, weekStart]);
  ...
  const cancelDuplicatedWeek = useCallback(async (): Promise<CancelDuplicatedWeekResult> => {
    if (isDemo) throw new Error("Connectez-vous avec un compte administrateur pour annuler une duplication.");
    if (!duplicateWeekUndoInfo) throw new Error("Cette duplication n’est plus annulable car la semaine cible a été modifiée ou publiée.");
    const result = await cancelDuplicatedWeekMutation.mutateAsync({ weekStart: duplicateWeekUndoInfo.sourceWeekStart });
    setWeekStart(result.sourceWeekStart);
    return result;
  }, [cancelDuplicatedWeekMutation, duplicateWeekUndoInfo, isDemo]);
  ...
  const value = useMemo<PlanningContextValue>(() => ({ isDemo, role, weekStart, snapshot, loading: isAuthenticated && (remoteQuery.isLoading || authQuery.isLoading), isAdmin: role === "admin", showOnlyMine, personalMember, visibleShifts, setWeekStart, changeWeek, setDemoRole: switchDemoRole, setShowOnlyMine, createShift, importPlanningExcel, updateShift, deleteShift, clearDraftWeek, createStaffMember, updateStaffMember, regenerateStaffCode, setStaffActive, duplicateWeekToNext, duplicateWeekUndoInfo, cancelDuplicatedWeek, weekTemplates, saveWeekAsTemplate, renameWeekTemplate, deleteWeekTemplate, applyWeekTemplate, publishWeek, createUnavailability, deleteUnavailability, signIn, notifications, unreadNotificationCount, markNotificationRead, publicationCheck, checkBeforePublish, user, login, logout }), [applyWeekTemplate, authQuery.isLoading, cancelDuplicatedWeek, changeWeek, checkBeforePublish, clearDraftWeek, createShift, createStaffMember, updateStaffMember, regenerateStaffCode, setStaffActive, deleteWeekTemplate, duplicateWeekToNext, duplicateWeekUndoInfo, createUnavailability, deleteShift, deleteUnavailability, importPlanningExcel, isAuthenticated, isDemo, login, logout, markNotificationRead, notifications, personalMember, publicationCheck, publishWeek, remoteQuery.isLoading, renameWeekTemplate, role, saveWeekAsTemplate, showOnlyMine, signIn, snapshot, switchDemoRole, unreadNotificationCount, updateShift, user, visibleShifts, weekStart, weekTemplates]);
